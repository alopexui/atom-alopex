...
const ALOPEX_HOME ='https://ui.alopex.io/'
export default {
  ...
  config:{
    ...
    showAlwaysTreeView:{
      ...
      type: 'boolean',
      default: true,
      description: 'Show Always Tree-View when opening a new Atom window.'
    },
  ...
  activate(state) {
    ...
    //  Register command that toggles this view
      ...
      this.subscriptions.add(atom.commands.add('.tree-view .selected',{
          'atom-alopex:createAlopexTemplateD': (e) => this.createAlopexTemplateD(e)
      }));
      this.subscriptions.add(atom.commands.add('.tree-view .selected',{
          'atom-alopex:createAlopexTemplateE': (e) => this.createAlopexTemplateE(e)
      }));
     ...
     atom.packages.activatePackage('tree-view').then(treeViewPkg => {
        ...
        //Atom 구동 시 setting 값에 따라 tree-view 열기
        if (atom.config.get('atom-alopex.showAlwaysTreeView')) {
          if(this.treeview){
            this.treeview.show();
          }else{
...
//    };
  ...
  createAlopexUIUXStandard(e){
    ...
    if(elClass.indexOf('directory') != -1){
      ...
      this.downloadAndExtractFile(ALOPEX_HOME+"asset/demo/alopex-uiux-standard.zip","alopex-uiux-standard.zip",extractDest,true)
    }else{
  ...
  createAlopexTemplateA(e){
    ...
    if(elClass.indexOf('directory') != -1){
      ...
      this.downloadAndExtractFile(ALOPEX_HOME+"asset/publishing/alopex-ui-templateA.zip","alopex-ui-templateA.zip",extractDest,true)
    }else{
      atom.notifications.addError ('An error has ocurred',{
        detail: "Template destination is not directory, you must select directory!",
        dismissable: true
 ...
 createAlopexTemplateB(e){
   ...
   if(elClass.indexOf('directory') != -1){
     ...
     this.downloadAndExtractFile(ALOPEX_HOME+"asset/publishing/alopex-ui-templateB.zip","alopex-ui-templateB.zip",extractDest,true)
   }else{
     atom.notifications.addError ('An error has ocurred',{
       detail: "Template destination is not directory, you must select directory!",
       dismissable: true
 ...
 createAlopexTemplateC(e){
   ...
   if(elClass.indexOf('directory') != -1){
     ...
     this.downloadAndExtractFile(ALOPEX_HOME+"asset/publishing/alopex-ui-templateC.zip","alopex-ui-templateC.zip",extractDest,true)
   }else{
     atom.notifications.addError ('An error has ocurred',{
       detail: "Template destination is not directory, you must select directory!",
       dismissable: true
 ...
 createAlopexTemplateD(e){

   let elClass = e.currentTarget.className;
   if(elClass.indexOf('directory') != -1){
     let extractDest = path.join (e.currentTarget.getPath(), 'alopex-ui-templateD');
     this.downloadAndExtractFile(ALOPEX_HOME+"asset/publishing/alopex-ui-templateD.zip","alopex-ui-templateD.zip",extractDest,true)
   }else{
     atom.notifications.addError ('An error has ocurred',{
       detail: "Template destination is not directory, you must select directory!",
       dismissable: true
 ...
 createAlopexTemplateE(e){

   let elClass = e.currentTarget.className;
   if(elClass.indexOf('directory') != -1){
     let extractDest = path.join (e.currentTarget.getPath(), 'alopex-ui-templateE');
     this.downloadAndExtractFile(ALOPEX_HOME+"asset/publishing/alopex-ui-templateE.zip","alopex-ui-templateE.zip",extractDest,true)
   }else{
     atom.notifications.addError ('An error has ocurred',{
       detail: "Template destination is not directory, you must select directory!",
       dismissable: true
