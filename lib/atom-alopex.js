...
export default {
  ...
  config:{
    ...
    useDoubleClickOnFileSelect:{
      order: 5,
      type:'boolean',
      default : true,
      description : 'Double-click on a file/directory to open in Tree-View'
    }
  },
  activate(state) {
    ...
    //  Register command that toggles this view
     ...
     atom.packages.activatePackage('tree-view').then(treeViewPkg => {
        ...
        //tree-view 에서 더블클릭으로 파일/디렉토리 열기 설정
        if(atom.config.get('atom-alopex.useDoubleClickOnFileSelect')){
          if(this.treeview){
